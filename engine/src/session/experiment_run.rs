...
pub fn run_experiment_path(ctx: StandardHappyCtx) -> anyhow::Result<()> {
    ...
    let score_cell = {
        ...
        if crate::kv::eviction::stage_registry::stage_is_score_based(policy) {
            ...
            // GPU-side accumulator init (OpenCL/CUDA) — mirrors `eval_setup`/`chat`. Arming the GPU
            ...
            // a score-fed eviction reads them, so the keep-set matches the CPU path. Log the outcome
            // (eval_setup 동형, chat 의 `let _ =` 와 의도적으로 다름): bench 는 measurement tool 이라 init
            // 실패 시 silent CPU-accumulate 폴백이 이 fix 가 없애려는 ~2× 로 보여 측정을 오염시킨다.
            // `arm_gpu_score_acc` 는 Ok 일 때만 set_active 하므로 실패 후 stale buffer 무장도 피한다.
