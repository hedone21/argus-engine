...
//! 공유(정직한 단일 핸들).
//!
//! **registry 병합 없음**: stage 는 `DYN_REGISTRY`, format 은 `DYN_FORMAT_REGISTRY` 로 분리
...
pub fn register_dynamic_plugins(paths: &[PathBuf]) -> Result<()> {
    for path in paths {
        ...
        if stages + formats + backends == 0 {
            anyhow::bail!(
                "plugin {}: 0 capabilities registered (register_kv_stages_v2·register_kv_formats_v2·register_backend_caps_v2 absent — export_plugin! missing?)",
                path.display()
            );
        }
    }
