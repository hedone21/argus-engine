...
//! NoEviction × {F32, F16, Q4_0} (per-head H2O+ · 가중 merge D2O 는 §9.1-EVICT-DEFER;
//! StreamingLLM 은 streaming-llm 플러그인으로 추출 — plan-only 라 World-A 등가 대상 아님).
//!
...
use crate::kv::eviction::{EvictionPolicy, H2OPolicy, NoEvictionPolicy, SlidingWindowPolicy};
    ...
    F: Fn(&P, &mut KVCache),
{
    ...
    // Path B: plan_keep → execute_kv_plan (retarget — compact 폐기).
    let mut cache_b = make_cache(dtype, n_tokens);
    let (keep, merges) = policy
        .plan_keep(n_tokens, target_len, importance)
        .unwrap_or_else(|| panic!("{label}: plan_keep returned None (policy unsupported)"));
