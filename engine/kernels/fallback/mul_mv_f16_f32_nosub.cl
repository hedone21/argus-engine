...
// Row-strided variant for tensor-partition column slices (ticket 021): row `r` of the weight
// starts `ld` halves after row `r-1` (instead of `ne00`), so `W[:, 0..ne00)` of a `[ne01, ld]`
// matrix is read in place. Arg 15 = `ld`; everything else is `kernel_mul_mat_f16_f32`.
REQD_SUBGROUP_SIZE
kernel void kernel_mul_mat_f16_f32_ld(
        global void * src0,
        ulong offset0,
        global float * src1,
        ulong offset1,
    ...
    ulong x_base = (ulong)first_row * ld
                  + (ulong)(i12/r2) * ((ulong)ld * ne01)
                  + (ulong)(i13/r3) * ((ulong)ld * ne01 * ne02);

    global half * xr0 = x + x_base;
    global half * xr1 = xr0 + ld;
    global half * xr2 = xr1 + ld;
    global half * xr3 = xr2 + ld;
    global float * yr = y + r1 * ne10 + im * ne00 * ne1;
