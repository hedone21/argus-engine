...
|----|-------------|------------------------|
| **stage** | `KVCacheStage`(제거/병합) 또는 `KVReadStage`(읽기) | 어떤 토큰을 남기고 어떻게 병합할지, 또는 attention에서 어떤 토큰/페이지를 읽을지 |
...
- 안정 버전 Rust 툴체인과 클론한 `argus-engine` 워크스페이스.
- 환경이 멀쩡한지 확인하는 베이스라인 빌드 한 번:
  ```bash
  cargo build --release
  ```
- 추론을 돌릴 모델(CLI 기본값은 `models/llama3.2-1b`). 엔진이 이미 로드하는 모델이면 무엇이든
  되고, 작성하는 플러그인은 모델과 무관합니다. 레포에는 모델이 동봉돼 있지 않으니, 받거나
...
> ABI는 `technique-api` 버전 간에 안정적이지 않습니다. 빠른 시작에는 해당 없지만(당신은 안전한
...
> 플러그인은 엔진과 같은 프로파일로 빌드하세요(여기선 둘 다 `--release`). 엔진이 `dlopen`할
> 때 ABI가 맞아야 합니다.
...
방금 플래그 하나로 엔진이 로드하는 KV 캐시 포맷을 더했습니다. 엔진 재빌드는 없었습니다.

---

## 플러그인 해부

모든 플러그인은 같은 두 부분 모양입니다. 트레이트를 구현하고, 한 줄로 등록합니다.
...
technique_api::register_kv_format!("my_format", || Box::new(MyFormat));
technique_api::export_plugin!();   // 동적 `.so` 경로에만 필요
```
...
그래서 한 줄이 양쪽을 다 배선하고, 어느 쪽이 활성인지는 크레이트를 어떻게 빌드하느냐(평범한
...
"선택자 종류"와 "도는 곳"은 트레이트만큼 중요합니다. 당신이 만든 플러그인을 지금 가진
바이너리에서 실제로 부를 수 있는지를 결정하니까요.
...
| **stage**(읽기) | `KVReadStage` | `Option<KVReadPlan>` | 직접 `#[distributed_slice(KV_READ_STAGES)]` | `--read-stage <name>` | 자유 문자열 | 아니오(정적 전용) | `argus-cli` | 프로덕션(`quest`) |
...
세 축 모두 등록된 기법을 이름으로, 엔진 수정 0으로 선택합니다. 차이는 *어느 바이너리*에서 돌고
플래그 철자가 무엇이냐뿐입니다:
...
2. stage와 backend-capability는 `argus-bench` / `argus-eval`에서 이름으로 선택합니다. 플러그인
   ...
   삽니다. 그리고 resilience manager는 CLI로 고른 stage를 (evict 타이밍/비율로) 구동합니다.
   다만 런타임에 *이름으로 기법을 전환*하는 건 아직 안 됩니다(`argus-shared` 프로토콜 추가 필요).
   자세한 건 [stage 사용법](#사용법-제거병합-stage-추가)을 보세요.

> 이 세 축 외에, 실험적인 **weight** 축(`WeightStage` / `WEIGHT_STAGES`, 런타임 가중치 swap용)도 있습니다.
...
그 위의 기본 슈가입니다. stage는 이 읽기들로 자기 지표를 직접 계산합니다. 예컨대 내장
...
> 일부 `technique-api` 문서 주석은 선택자를 여전히 `--eviction-policy <name>`이라 부릅니다. 낡은
...
**read stage**는 stage 축의 '읽기' 종류로, 저장된 것을 바꾸지 않고 *무엇을 읽을지*(쿼리 인지로
추린 토큰/페이지)를 결정합니다. `None`은 "전체 읽기"(정확함, 기본값)라서, read stage는 옵트인
근사일 뿐 절대 정확성을 위협하지 않습니다.
...
// read stage에는 편의 매크로가 없습니다: 등록 항목을 직접 제출합니다.
...
- 자유 문자열 플래그 `--read-stage my_read`로 선택하고, 출하된 `argus-cli`에서 디코드까지
  닿습니다. (기본 미설정 = 전체 읽기 = 플러그인 없는 것과 바이트 동일.)
- 정적 전용. read stage에는 동적 `.so` 진입 심볼이 없으므로, read stage는 **force-link** 해야
...
| ABI 안정성 | 해당 없음(같은 컴파일) | **취약**: `technique-api` 버전이 맞아야 함 |
| 충돌 격리 | 프로세스 공유 | 프로세스 공유(샌드박스 없음) |
| 배포 주체 | 엔진 메인테이너 | 누구나, 독립 `.so`로 |
| 지원 축 | 세 축 모두 | format, stage, backend-cap (read는 정적 전용) |
...
- `export_plugin!()`은 `.so`당 동일한 세 개의 `#[no_mangle] extern "C"` 진입 심볼을 항상
  ...
  `register_backend_caps_v2`. (read stage 심볼은 없습니다. read는 정적으로 남습니다.) 안 쓴
...
| 증상 | 원인 | 해결 |
|------|------|------|
| 플러그인은 컴파일되는데 엔진이 이름을 못 찾음 | 크레이트가 호스트 링크 그래프에 없거나, 등록이 dead-code로 제거됨 | 정적 경로면 경로 의존성과 `use my_crate as _;` force-link 줄을 추가; 동적 경로면 `--load-plugin <.so>` 전달 |
...
| `.so`는 로드되나 `0 capabilities registered (… export_plugin! missing?)` | `technique_api::export_plugin!()`을 깜빡함 | `.so`당 한 번 추가([`example-no-export`](../crates/techniques/example-no-export)) |
| `.so` 거부: `abi_version … != expected …` | 다른 `technique-api` 버전으로 빌드된 플러그인 | 엔진 버전에 맞춰 재빌드(ABI는 버전 간 안정적이지 않음) |
...
- [ ] 의존성은 정확히 `technique-api`(경로 또는 버전) + `linkme = "0.3"`. 엔진 의존성 없음.
- [ ] 등록 발화를 단언하는 단위 테스트(`find_kv_format(...).is_some()` 등).
- [ ] `.so`를 빌드한 `technique-api` 버전을 명시. 동적 ABI는 버전 간 안정적이지 않음.
...
- **레퍼런스**: 트레이트/매크로 표면 전체와 문서 주석:
  [`crates/technique-api/src/lib.rs`](../crates/technique-api/src/lib.rs) (`cargo doc
  -p technique-api --open`)과 크레이트 자체
