...
A guide for adding a **KV-cache technique** to Argus — a new eviction policy, a new
storage format, a query-aware read strategy — **without forking the engine**. You write
one trait impl in a small crate of your own, it registers itself, and the engine picks it
up by name. This is the same `argus-extension-api` surface the built-in techniques (`q4_0`,
...
own crate, self-registered via [`linkme`](https://docs.rs/linkme), loaded into the real
inference binary with `--load-plugin`, and selected at runtime with `--kv-format
my_format` — **with no rebuild of the engine.**
...
- [The four extension axes](#the-four-extension-axes)
...
One idea carries the whole system: **the plugin decides, the engine executes.** Your code
never touches a KV buffer or a GPU command queue directly. It reads a read-only view of
the cache and returns a *plan* (or a *descriptor*); the engine owns all mutation. That is
what keeps a technique crate decoupled from engine internals — and what lets the same code
link statically or load as a `.so`.
...
|------|---------------|------------------------|
| **stage** | `KVCacheStage` | which tokens to keep + how to merge (eviction, H2O, D2O) |
| **format** | `KVFormat` | the byte layout / precision your cache is stored in (q4_0, KIVI) |
| **read** | `KVReadStage` | which tokens/pages to read for attention (Quest) |
| **backend-capability** | `KiviAttentionBackend` | a specialized fused GPU kernel |

Each axis is independent: adding a member to one touches **zero** code on the others, and
**zero** code in the engine core. That is the payoff — and the rest of this guide is how to
collect it.

> Why no central list to edit? Each technique submits itself to a global registry slice at
> **link time** via `linkme`. The engine reads the slice; there is no `match` arm to add.
> The deeper "why three axes" model lives in [`CONTEXT.md`](../CONTEXT.md) — you do **not**
> need it to finish the Quickstart.
...
- A model to run inference against (the CLI defaults to `models/llama3.2-1b`). Any model
  ...
  ship a model — see the [README](../README.md) for how to obtain or convert one. **Steps
  1–3 of the Quickstart need no model** (they prove registration), so you can do the whole
  build-and-verify loop before you have one; only Step 4 runs inference.

The Quickstart is **CPU-only and host-runnable** — no GPU, no device. (GPU/on-device
specifics are out of scope here.)
...
Replace the body of `crates/techniques/my-format/src/lib.rs` with this complete file —
the only thing that matters is that the **name string** is unique:
...
The next sections explain what each piece does and how the other axes differ.

---

## Anatomy of a plugin

Every plugin is the same two-part shape: **implement a trait**, then **register it in one
line**.
...
This is the honest capability map. **"Selector kind" and "Runs on" matter as much as the
trait** — they decide whether the plugin you write can actually be invoked from the binary
you have.
...
> There is a fifth, **weight** axis (`WeightStage` / `WEIGHT_STAGES`, for runtime weight
> swap). Its trait surface exists, is unit-tested, and can be registered today via the raw
...
own metric** from these reads — e.g. the built-in `caote`
...
The **read** axis decides *what to read* for attention — a query-aware subset of tokens or
pages — without changing what's stored. `None` means "full read" (exact, the default), so a
read stage is an opt-in approximation, never a correctness risk.
...
// The read axis has NO convenience macro — submit the registration entry directly.
...
Knowing *when* your methods run is the difference between a plugin that works and one that
quietly destroys decode throughput.
...
| Axes supported | all four | format, stage, backend-cap — **not read** |

**How a compile-time registry picks up a runtime plugin** (the apparent paradox): the two
paths register at *different times*. Static elements are gathered into the linker section at
**link time**; a `.so`'s capabilities are registered when **`dlopen` runs**. At lookup
time the engine checks the static slice first, then falls back to the dynamic registry — so
both resolve through one source-agnostic `find_*` / `make_*` seam. One line
(`register_*!` + `export_plugin!()`), gated by the `plugin-cdylib` feature and
`crate-type = ["cdylib", "rlib"]`, serves both.
...
- `export_plugin!()` always emits the same three `#[no_mangle] extern "C"` entry symbols per
  ...
  `register_backend_caps_v2`. (There is **no** read-axis symbol — read stays static.) An
...
This is how the always-linked built-ins (`quest`, `synth-q4-format`) are wired — and the
...
- [ ] `Cargo.toml`: `crate-type = ["cdylib", "rlib"]` and a `plugin-cdylib = []` feature
      (off by default).
- [ ] Dependencies are exactly `argus-extension-api` (path or version) + `linkme = "0.3"` — no
      engine dependency.
